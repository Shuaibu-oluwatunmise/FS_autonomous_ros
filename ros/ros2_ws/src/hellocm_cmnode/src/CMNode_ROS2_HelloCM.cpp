...
int CMNodeHelloCM::userTestrunStartAtBegin() {
  ...
  if (VehicleInfo_Err == 0) {
    ...
    { /* set up CameraRSI publishing nodes */
      ...
      /* Collect cameras and group by cluster */
      struct CameraInfo {
        char* name;
        int number;
        double* pos;
      ...
      std::map<int, std::vector<int>> cluster_to_cameras; // cluster_idx -> [camera indices in cameras vector]
      ...
      for (idxS = 0; idxS < CMNode.Misc.nSensors; idxS++) {
        ...
        if (!strcmp(str, "CameraRSI")) {
          ...
          if (camera_active) {
            ...
            ci.number = camera_no;
            sprintf(sbuf, "Sensor.%d.pos", idxS);
            ci.pos    = iGetFixedTableOpt2(Inf_Vehicle, sbuf, def3c, 3, 1);
            sprintf(sbuf, "Sensor.%d.rot", idxS);
            ci.rot    = iGetFixedTableOpt2(Inf_Vehicle, sbuf, def3c, 3, 1);
            sprintf(sbuf, "Sensor.Param.%d.FoV", idxP);
            ci.FoV    = iGetDblOpt(Inf_Vehicle, sbuf, 60);
            sprintf(sbuf, "Sensor.Param.%d.Resolution", idxP);
            ci.Resolution = iGetFixedTableOpt2(Inf_Vehicle, sbuf, def2c, 2, 1);
            sprintf(sbuf, "SensorCluster.%d.Socket", idxC);
            ci.Socket = iGetIntOpt(Inf_Vehicle, sbuf, 0);
            ci.ClusterIdx = idxC;

            int cam_idx = cameras.size();
            cameras.push_back(ci);
            cluster_to_cameras[idxC].push_back(cam_idx);

            /* Also store in CMNode for other uses */
            Camera.name = strdup(ci.name);
            strcpy(Camera.name, ci.name);
            ...
            Camera.FoV = ci.FoV;
            Camera.Resolution = ci.Resolution;
            sprintf(sbuf, "Sensor.Param.%d.CycleOffset", idxP);
            Camera.nCycleOffset = iGetIntOpt(Inf_Vehicle, sbuf, 0);
            Camera.Socket = ci.Socket;
      ...
      /* Launch RSDS clients - one per cluster */
      std::set<int> launched_clusters;
      for (auto& pair : cluster_to_cameras) {
        int cluster_idx = pair.first;
        auto& cam_indices = pair.second;

        if (launched_clusters.count(cluster_idx)) continue;
        launched_clusters.insert(cluster_idx);

        CameraInfo& primary = cameras[cam_indices[0]];

        if (cam_indices.size() == 1) {
          /* Single camera in cluster - standard launch */
          sprintf(sbuf, "ros2 launch carmaker_rsds_client carmaker_rsds_client.launch.py \
            ...
            primary.name,
            primary.Socket,
            primary.pos[0],primary.pos[1],primary.pos[2],primary.rot[0],primary.rot[1],primary.rot[2],
            primary.FoV,
            primary.Resolution[0],
            primary.Resolution[1]);
          system(sbuf);
        } else if (cam_indices.size() >= 2) {
          /* Multi-camera cluster - launch with secondary camera parameters */
          CameraInfo& secondary = cameras[cam_indices[1]];
          sprintf(sbuf, "ros2 launch carmaker_rsds_client carmaker_rsds_client.launch.py \
            camera_no:=%d \
            camera_name:=%s \
            rsds_port:=%d \
            ...
            height:=%f \
            secondary_channel:=%d \
            secondary_camera_name:=%s \
            secondary_camera_frame:=CARS00 \
            secondary_param_trans_rot:=[%f,%f,%f,%f,%f,%f] \
            secondary_fov_deg:=%f \
            secondary_width:=%f \
            secondary_height:=%f &",
            primary.number,
            ...
            secondary.number,
            secondary.name,
            secondary.pos[0],secondary.pos[1],secondary.pos[2],secondary.rot[0],secondary.rot[1],secondary.rot[2],
            secondary.FoV,
            secondary.Resolution[0],
            secondary.Resolution[1]);
          system(sbuf);
        }
      }
    } // set up CameraRSI publishing nodes
